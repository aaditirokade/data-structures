...
app.get('/ss', function(req, res) {
    ...
    client.query(q, (qerr, qres) => {
        if (qerr) { throw qerr }
            ...
            // var resp = s1x + JSON.stringify(qres.rows) + s2x; //html in response - vinal html
            var resp = s1x + JSON.stringify(qres.rows); //html in response - vinal html
            res.send(resp);
            client.end();
...
app.get('/aameetings', function(req, res) {
    ...
    client.query(thisQuery, (qerr, qres) => {
        if (qerr) { throw qerr }
            ...
            res.send(qres.rows);  //what's this -
            client.end();
